...
## AUTH-FIX-1 (2026-06-23): bearer-token identity for the human/web path

The API now accepts a **verified bearer JWT** for human identity, alongside `wsp_` keys for machines. See `docs/audits/AUTH-FIX-1.md`. **Shipped to a PR only — NOT merged/deployed** (owner-controlled 3-step cutover; `AUTH_API_SECRET` must be set identically in the web + API Railway services first).

- **API verify:** `packages/api/src/lib/api-token.ts` `verifyApiToken()` (jose HS256, `AUTH_API_SECRET`, issuer `workspace-web` / audience `workspace-api`). `middleware/auth.ts` is **dual-accept**: a valid bearer JWT authenticates by its `sub` and takes precedence over `x-user-id`; an absent/invalid bearer **falls through** to the legacy `x-user-id` path (a `// PHASE 3:` marker shows where that becomes a 401).
- **Web mint:** `apps/web/src/app/api/auth/api-token/route.ts` reads the NextAuth session server-side (`auth()`) and signs a 30-min token with `AUTH_API_SECRET` (secret stays server-side). `api-client.ts` mints+caches it (re-mints <5 min to expiry) and sends `Authorization: Bearer …` instead of `x-user-id` on the real path (dev path unchanged).
- **Impersonation + cross-tenant read: CLOSED by security-gate-1 (2026-07-25) — see the section below.** (This AUTH-FIX-1 section describes the earlier dual-accept phase; the `// PHASE 3:` marker and the header-fallthrough it references are gone.)
- **CORS unchanged:** hono reflects requested headers when `allowHeaders` is empty, so `Authorization` already passes cross-origin (like `x-user-id` does); an explicit allow-list is optional hardening.

---

## security-gate-1 (2026-07-25): Phase 3 + the workspace-membership guard (multi-tenant lock)

The two holes that blocked a second real user are closed, so Follow can take teams. See `docs/audits/` and the PR. **The standing "no 2nd user until both close" gate is now satisfied.**

- **Phase 3 — identity is verified-only.** `middleware/auth.ts` no longer trusts `x-user-id`. Identity comes from a verified bearer JWT (humans) or a `wsp_` key (machines, via `flexAuth`). A bare/forged header, or an invalid/expired bearer, is a hard **401**. The legacy header path survives _only_ under `DEV_BYPASS_AUTH` (local) or the break-glass env flag **`AUTH_ACCEPT_LEGACY_USER_HEADER=true`** (emergency rollback without a redeploy — startup warns if it's on in prod; never leave it on). Deploy scripts (`deploy/smoke.ts`, `deploy/e2e-smoke.ts`) mint a bearer from `AUTH_API_SECRET` instead of sending `x-user-id`. The two dead header-sending web hooks (`use-track-event`, `use-signal-capture`, both posting to CLEANUP-1-removed endpoints) were deleted.
- **The membership guard — `services/workspace-access.ts`.** `checkWorkspaceAccess(userId, workspaceId)` = workspace exists (not soft-deleted) AND caller is owner OR active `workspace_members` row (the same predicate `set_scope` uses; non-UUID ids like the legacy `'default'` → 404, never a DB cast error). Forms: `assertWorkspaceAccess(c, workspaceId)` (route handlers → returns the error Response or null), `requireWorkspaceMembership` (router-level, header-scoped routers), `checkWorkspaceAccessForTool` (MCP). Applied at **every** boundary that takes a caller-supplied `workspaceId` (header/body/query) or a by-id resource with a workspace — ~25 route files + the MCP transport. MCP is guarded once at the HTTP boundary (`/mcp` GET/POST + `/api/mcp-rest`) since every session-scoped tool reads `session.workspaceId`; the `activeProjectId` rehydration TOCTOU (`transport.ts`) is re-checked on session create.
- **Also locked (were unauthenticated):** `/api/users` (was fully open incl. `DELETE /:id` → any account) now requires auth — cross-user reads/create are operator-only (`dashboardTokenMiddleware`), update/delete are self-only; `/api/dev/graph` is gated behind the dashboard token (the console sends it via `authedFetch`); `/api/debug/*` is registered only outside production. `admin.ts` now checks membership of the workspace it MUTATES (body), not the header workspace.
- **Break-glass:** if a header client (extension/mobile/GWS) breaks in prod, set `AUTH_ACCEPT_LEGACY_USER_HEADER=true` via the Railway CLI + restart to restore the legacy path, migrate the client to a JWT/key, then unset. Prod MCP (`wsp_` keys) and web (JWT) are unaffected either way.
