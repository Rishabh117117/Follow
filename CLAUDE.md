...
- **NOT closed yet (impersonation):** Phase 3 is **deferred** — the API still trusts `x-user-id` when no valid bearer is present. Closing it requires migrating the other header clients (`apps/extension`, `apps/gws-addon`, `apps/gws-extension`, `apps/mobile`, `apps/web/public/extension/*.js`) to the JWT, then removing the fallthrough (`auth.ts` marker) + the `flexAuth` header fallback (`api-key-auth.ts:70-84`). Cross-tenant read (the `assertWorkspaceAccess` membership guard) is also still open. **Both must ship before a second real user.**
